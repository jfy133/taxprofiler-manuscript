...
The fixed set of software are controlled through the use of the conda package manager and containers (e.g., docker, singularity) from the stable Bioconda [@Gruning2018-vr] and BioContainers [@Da_Veiga_Leprevost2017-gl] repositories.
...
: List of nf-core/taxprofiler supported taxonomic classifers profilers as of version 1.1 and their method and support input database types.
...
The pipeline provides high efficiency, particularly during the metagenomic classification stage, through the inherent parallalisation provided by Nextflow.
...
Here we will review some of the functional similarities and differences between such of these pipelines.
Some important advantages over existing pipelines and workflows <!--Name those pipelines--> is that nf-core/taxprofile is portable and easy to install, can be accessible to users not so experienced with command-line by offering a graphical user interface, uses Docker and Singularity containers to ensure reproducibility.
In contrast pipelines such as <!-- examples -->, still require manual install of the tools in their workflow.
Many of the current workflows and pipelines available are already use multiple taxonomic classifiers in their implementation and to our knowledge the crucial factor of database choice still remains a challenge.
nf-core/taxprofiler partly adresses the challenge of database choice that has been previously reported <!--reference here from one of the articles--> by enabling the use many databases in parallel.
