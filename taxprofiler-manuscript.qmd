...
An example nf-core command line execution of the pipeline can be seen in Supplementary Data in figure (@lst-example-cmd)
...
The Supplementary Data section provides a comprehensive overview of the various criteria and comparative analyses applied to the five pipelines.
...
Two input files are supplied: one file specifying paths of FASTQ files of metagenomic samples and necessary metadata for preprocessing (such as sample ID and sequencing platform), and the second file specifying paths to the user-defined databases with per-database classification parameters.
