...
Many of the current workflows and pipelines available are already using multiple taxonomic classifiers in their implementation and to our knowledge the crucial factor of database choice still remains a challenge.
Some important advantages over existing pipelines and workflows <!--Name those pipelines--> is that nf-core/taxprofile is portable and easy to install, can be accessible to users not so experienced with command-line by offering a graphical user interface, uses docker and singularity containers to ensure reproducibility, and adresses the challenge of database choice that has been previously reported <!--reference here from one of the articles--> by enabling to use many databases in parallel.


Finally, once an overall taxonomic profile is generated, researchers often wish to validate hits through more sensitive and accurate methods such as with read-mapping alignment. The pipeline could be easily extended to have a validation step where, utilising Nextflow's execution parallelism, the input sequences could be aligned back to the reference genomes of only those species with hits from the taxonomic classification. This step could be used in different contexts. It can be particularly used for researchers in palaeogenomics, where alignment information can be used to authenticate ancient DNA within their samples and in clinical metagenomics to identify potential pathogens.
